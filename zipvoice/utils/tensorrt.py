# start zipvoice/utils/tensorrt.py
...
class TrtContextWrapper:
    ...
    def __init__(self, trt_engine: Any, trt_concurrent: int = 1, device: str = "cuda:0"):
        ...
        for _ in range(trt_concurrent):
            ...
            if trt_context is None:
                msg = (
                    f"failed to create trt context, maybe not enough CUDA memory, "
                    f"try reduce current trt concurrent {trt_concurrent}"
                )
                raise RuntimeError(msg)
            self.trt_context_pool.put([trt_context, trt_stream])
        if self.trt_context_pool.empty() is not False:
            msg = "no avaialbe estimator context"
            raise RuntimeError(msg)
        self.feat_dim = 100
    ...
    def __call__(
        ...
        with stream:
            ...
            input_data_ptrs = [
                ...
                padding_mask.contiguous().data_ptr(),
            ]
            if guidance_scale is not None:
            ...
            # run trt engine
            if estimator.execute_async_v3(torch.cuda.current_stream().cuda_stream) is not True:
...
def load_trt(model: nn.Module, trt_model: str, trt_concurrent: int = 1):
    ...
    if not os.path.exists(trt_model):
        msg = "Please export trt model first."
        raise RuntimeError(msg)
