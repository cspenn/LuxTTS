# start zipvoice/bin/train_zipvoice_distill.py
#!/usr/bin/env python3
...
app = typer.Typer(
    help="Train a ZipVoice-Distill model starting from a ZipVoice model.",
...
def ema(new_model, ema_model, decay):
    ...
    for key in new_model_dict:
        ema_model_dict[key].data.copy_(ema_model_dict[key].data * decay + new_model_dict[key].data * (1 - decay))


def compute_fbank_loss(  # noqa: PLR0913
    params: AttributeDict,
    model: nn.Module | DDP,
    teacher_model: nn.Module | DDP,
    features: Tensor,
    ...
    t_value = random.random()  # noqa: S311
    ...
    t_delta_fix = random.uniform(0.0, min(0.3, 1 - t_value))  # noqa: S311
    t_delta_ema = random.uniform(0.0, min(0.3, 1 - t_value - t_delta_fix))  # noqa: S311
...
def run(rank, world_size, args):  # noqa: C901, PLR0912, PLR0915
    ...
    if params.teacher_model is None:
        msg = "A teacher model checkpoint is required"
        raise ValueError(msg)
    log.info("loading_pretrained_model", checkpoint=str(params.teacher_model))
    ...
    if params.start_epoch > 1:
        log.info("resuming_from_epoch", epoch=params.start_epoch)
        if params.distill_stage == "first":
    ...
    log.info(
        ...
        pct=f"{num_trainable / num_param * 100:.3f}%",
    )
...
def main(  # noqa: PLR0913
    ...
    num_epochs: int = typer.Option(1, "--num-epochs", help="Number of epochs to train."),  # noqa: B008
    ...
    teacher_model: str | None = typer.Option(None, "--teacher-model", help="Checkpoints of pre-trained teacher model"),  # noqa: B008
    exp_dir: str = typer.Option(  # noqa: B008
        "exp/zipvoice_distill",
        "--exp-dir",
        help="The experiment dir. It specifies the directory where all training related files, "
        "e.g., checkpoints, log, etc, are saved",
    ),
    base_lr: float = typer.Option(0.001, "--base-lr", help="The base learning rate."),  # noqa: B008
    ...
    ema_decay: float = typer.Option(  # noqa: B008
        0.9999, "--ema-decay", help="The EMA decay factor of target model in distillation."
    ),
    distill_stage: str | None = typer.Option(  # noqa: B008
        None, "--distill-stage", help="The stage of distillation. Choices: first, second."
    ),
    ...
    """Train a ZipVoice-Distill model using distributed data parallel training."""
    args = AttributeDict(
        {
            "world_size": world_size,
...
# end zipvoice/bin/train_zipvoice_distill.py
